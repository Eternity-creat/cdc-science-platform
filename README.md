...
- **Agent**（Python Agent）：意图解析、向量检索、写作技法规划、大纲生成与校验、正文生成（动态 prompt + 6 层写作知识注入）、三路并行质量检查（事实核查 + 规则校验 + 文风评估）、反思迭代与规则修正、文笔润色、图片生成
...
│       ├── skills/           # 17 个 Skill（流程 + 知识库 + 写作知识）
│       │   ├── flow/         # 15 个流程类 Skill
│       │   ├── wiki/         # 2 个知识库类 Skill
│       │   └── writing/      # 6 层写作知识体系（蓝图/画像/技法/质量基准）
│       ├── tools/            # 向量存储、内容格式化
